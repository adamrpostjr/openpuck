<script lang="ts">
	...
	import { supported } from '$lib/state/device.svelte';
...
<svelte:head><title>OpenPuck RF Sniffer</title></svelte:head>

<div class="bg-app-bg text-app-strong flex h-screen flex-col overflow-hidden">
	<header class="border-app-line bg-app-chrome flex shrink-0 flex-wrap items-center gap-3 border-b px-4 py-2">
		<span class="flex items-center gap-2 font-bold tracking-tight">
			<RadioIcon size={16} /> OpenPuck RF Sniffer
		</span>
		<span
			class="rounded-full px-2 py-0.5 text-xs font-semibold
			{sniffer.connected ? 'bg-success-100-900 text-success-700-300' : 'bg-error-100-900 text-error-700-300'}"
		>
			{sniffer.connected ? 'connected' : 'disconnected'}
		</span>
		{#if s}
		...
		{/if}
		<a class="text-primary-700-300 text-xs hover:underline" href="./index.html">← Config panel</a>
		<div class="flex-1"></div>
		<button type="button" class="btn preset-tonal-surface btn-sm" onclick={() => panels.toggle('logs')}>Logs</button>
		<button
			type="button"
		...
		>
			<PlugZapIcon size={14} />
			{sniffer.connected ? 'Reconnect' : 'Connect'}
		</button>
	</header>
	...
	{#if !supported}
		<div class="flex flex-1 items-center justify-center p-8 text-center">
			<div>
				<h1 class="mb-2 text-lg font-semibold">This browser can't talk to the sniffer</h1>
				<p class="text-app-muted text-sm">The sniffer needs WebUSB, available in Chrome and Edge.</p>
			</div>
		</div>
	{:else}
							...
							<tbody>
								{#each sniffer.rows as e (e.seq)}
									<tr class="border-app-line-soft border-b" class:opacity-60={!e.crc}>
										...
										<td class="py-0.5 pr-2 {e.dir === 'P→C' ? 'text-primary-700-300' : 'text-secondary-700-300'}"
											>{e.dir}</td
										>
