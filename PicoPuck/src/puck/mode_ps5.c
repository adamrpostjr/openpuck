...
static uint16_t ps5_build(int slot, uint8_t *out, uint8_t *rid)
{
	*rid = 0x01;
	...
	out[9] = ((b & TB_STEAM) ? 0x01 : 0) |
		 ...
		 ((b & TB_MUTE) ? 0x04 : 0);
	out[15] = g_in[slot].gx & 0xFF;  out[16] = g_in[slot].gx >> 8;
	out[17] = g_in[slot].gz & 0xFF;  out[18] = g_in[slot].gz >> 8;
	out[19] = (-g_in[slot].gy) & 0xFF; out[20] = (-g_in[slot].gy) >> 8;
	out[21] = g_in[slot].ax & 0xFF;  out[22] = g_in[slot].ax >> 8;
	out[23] = g_in[slot].ay & 0xFF;  out[24] = g_in[slot].ay >> 8;
	out[25] = g_in[slot].az & 0xFF;  out[26] = g_in[slot].az >> 8;
			...
			uint16_t reqlen)
{
	if (type != PP_HID_FEATURE || reqlen == 0)
		return 0;
	memset(buf, 0, reqlen);
	uint8_t mac[6] = { 0x00, 0x1B, 0xDC, 0x4F, 0x55, (uint8_t)(0x60 + slot) };
	switch (rid) {
	case 0x03:
		...
		return 47;
	case 0x05:
		if (reqlen < 40) return 0;
		ps_neutral_calib(buf);
		return 40;
	case 0x09:
		if (reqlen < 19) return 0;
		memcpy(buf, mac, 6);
		return 19;
	case 0x20:
...
const emu_mode_t emu_ps5 = {
	.vid = 0x054C, .pid = 0x0CE6, .bcd = 0x0110,
