...
#define SETTINGS_MAGIC 0x50503104u  // "PP" v4 (bond[] table; moved off the bond bank)
...
static void defaults(pp_cfg_t *c)
{
	...
	c->rumble_scale = 100;
	c->persist_mode = 0;
	c->chord[0] = MODE_PS5;    // B
	c->chord[1] = MODE_HIDGYRO;  // X
	c->chord[2] = MODE_SW_HORI;  // Y
	c->sw_pro_rate = 2;
	c->sw_gyro10 = 10;
	for (int et = 0; et < 4; et++) {
		...
		c->type[et].back[3] = 8;
		c->type[et].rumble = 1;
		c->type[et].ab_swap = (et == 1) ? 1 : 0;  // Switch = Nintendo layout
	}
}
...
static void commit(void)
{
	...
	memcpy(page, &st, sizeof(st) < sizeof(page) ? sizeof(st) : sizeof(page));
...
void settings_set_field(uint8_t field, uint8_t val)
{
	...
	if (field >= 40 && field < 40 + 4 * 9) {
		...
		switch (k) {
		case 0: case 1: case 2: case 3: t->back[k] = val; break;
		case 4: t->qam = val; break;
		case 5: t->ab_swap = val ? 1 : 0; break;
		case 6: t->pad_haptics = val ? 1 : 0; break;
		case 7: t->led_bright = val > 100 ? 100 : val; break;
		case 8: t->rumble = val ? 1 : 0; break;
		}
		commit();
		return;
	}
	switch (field) {
	case 1: s_cfg.m_div = val < 4 ? 4 : val; break;
	case 2: s_cfg.m_fric = val > 99 ? 99 : val; break;
	case 16: s_cfg.persist_mode = val ? 1 : 0; break;
	case 17: if (val <= MODE_MAX) s_cfg.chord[0] = val; break;
	case 18: if (val <= MODE_MAX) s_cfg.chord[1] = val; break;
	case 19: if (val <= MODE_MAX) s_cfg.chord[2] = val; break;
	case 22: s_cfg.rumble_scale = val; break;
	case 23: s_cfg.sw_pro_rate = val <= 2 ? val : 2; break;
	case 24: s_cfg.sw_gyro10 = (val >= 5 && val <= 30) ? val : 10; break;
	case 29: s_cfg.land_all_87 = val ? 1 : 0; break;
	default: return;  // unknown/ignored field — don't rewrite flash
	}
	commit();
}
...
{
	for (int i = 0; i < 4; i++)
		if (s_cfg.bond[i].used && memcmp(s_cfg.bond[i].addr, addr, 6) == 0)
			return i;
	return -1;
}
			    ...
			    uint8_t kind, const char name[16])
{
	...
	if (i < 0)
		for (int k = 0; k < 4; k++)
			if (!s_cfg.bond[k].used) { i = k; break; }
	if (i < 0)
