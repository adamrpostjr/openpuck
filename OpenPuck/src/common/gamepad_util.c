// gamepad_util.c — see gamepad_util.h.
//
...
{
	// Payload offsets = kernel buf[] index minus 1. buf[0..5] gyro bias stays
	// zero (caller memset). Symmetric ranges → non-zero host divisors.
	le16(buf + 6, 2844);
...
// ---- Steam trackpad → absolute touch (internal helpers) --------------------
...
uint8_t ps_face_nibble(uint32_t b, bool ab_swap)
{
	uint8_t f = 0;
	if (ab_swap) {
		if (b & TB_A)
